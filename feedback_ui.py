...
from PySide6.QtCore import Qt, Signal, QObject, QTimer, QSettings, QSize, QBuffer, QByteArray
from PySide6.QtGui import (
    QTextCursor, QIcon, QKeyEvent, QFont, QFontDatabase, QPalette, QColor,
    QPixmap, QClipboard, QTextDocumentFragment
)
...
class ImagePreviewWidget(QLabel):
    def __init__(self, pixmap: QPixmap, parent=None):
        ...
        self.setStyleSheet("border: 1px solid #666; margin: 2px;")
        self.setToolTip("Click to view full size")
        self.setCursor(Qt.PointingHandCursor)
        
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            # Create a dialog to show full-size image
            from PySide6.QtWidgets import QDialog, QVBoxLayout, QScrollArea, QPushButton
            ...
            full_image_label.setPixmap(self.original_pixmap)
            full_image_label.setAlignment(Qt.AlignCenter)
            ...
            # Set dialog size to fit image or screen, whichever is smaller
            screen_size = dialog.screen().availableGeometry().size()
            image_size = self.original_pixmap.size()
            dialog_width = min(image_size.width() + 50, screen_size.width() - 100)
            dialog_height = min(image_size.height() + 100, screen_size.height() - 100)
...
class FeedbackTextEdit(QTextEdit):
    def __init__(self, parent=None):
        super().__init__(parent)
        self.images = []  # Store base64 encoded images
        self.image_previews = []  # Store preview widgets
    ...
    def keyPressEvent(self, event: QKeyEvent):
        if event.key() == Qt.Key_Return and event.modifiers() == Qt.ControlModifier:
            ...
            # Handle Ctrl+V - check if clipboard has image
    ...
    def _paste_image(self):
        ...
        if not pixmap.isNull():
            ...
            preview_widget = ImagePreviewWidget(pixmap, self)
            self.image_previews.append(preview_widget)
            
            # Insert preview into the text edit
            ...
            # Move to end and add the preview
            cursor.movePosition(QTextCursor.End)
            self.setTextCursor(cursor)
            
            # We'll add preview to parent widget instead of directly in text
            self._add_image_preview(preview_widget)
    
    def _add_image_preview(self, preview_widget):
        # Find the parent FeedbackUI to add preview to the layout
        parent = self.parent()
        while parent and not isinstance(parent, FeedbackUI):
            parent = parent.parent()
        if parent:
            parent._add_image_preview(preview_widget)
...
class FeedbackUI(QMainWindow):
    def __init__(self, project_directory: str, prompt: str):
        ...
        self.setWindowTitle("Cursor Feedback MCP")
    ...
    def _create_ui(self):
        ...
        self.feedback_text.setPlaceholderText("Enter your feedback here (Ctrl+Enter to submit, Ctrl+V to paste images)")
        
        # Image preview section
        image_label = QLabel("Attached Images:")
        image_label.setVisible(False)
        self.image_label = image_label
        feedback_layout.addWidget(image_label)
        
        # Scroll area for image previews
        self.image_scroll_area = QScrollArea()
        self.image_scroll_area.setMaximumHeight(100)
        self.image_scroll_area.setWidgetResizable(True)
        self.image_scroll_area.setVisible(False)
        
        # Widget to contain image previews
        ...
        feedback_layout.addWidget(self.image_scroll_area)
    ...
    def _submit_feedback(self):
        self.feedback_result = FeedbackResult(
            ...
            images=self.feedback_text.images
        )
        self.close()
    ...
    def _add_image_preview(self, preview_widget):
        """Add an image preview widget to the image preview area"""
        self.image_layout.addWidget(preview_widget)
        
        # Show the image section if it was hidden
        if not self.image_scroll_area.isVisible():
